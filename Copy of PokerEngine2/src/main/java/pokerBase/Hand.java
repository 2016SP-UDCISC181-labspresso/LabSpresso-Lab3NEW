...
public class Hand {
	...
	private static boolean isStraight(ArrayList<Card> cards, Card highCard, ArrayList<eCardNo> wilds) {
		...
		boolean bAce = false;
		//for if you have wilds
		int index = (cards.size()-1)-wilds.size();//the first index that is not a wild card
		eSuit compairTo = cards.get(index).geteSuit();//compare each subsequent card's suit to this one (it's the first one you encounter)
		...
		//some wilds, some not
		for (int i = index;i< (cards.size()-1);i++){ //for every card that is not a wild 
			if(cards.get(i).geteSuit() == compairTo){//if the suit is equal to the compairTo suit...
				continue;//do another iteration
			}
			else{return bIsStraight;}//if the suit doesn't match up, return false
			
		}
		highCard.seteRank(eRank.ACE);
		highCard.seteSuit(eSuit.SPADES);
		bIsStraight = true; //if it goes through the whole loop w/o returning false, you have yourself a Straight!
		return bIsStraight;
	}
//	
//		
...
//		return bIsStraight;
	...
	public static boolean isHandPair(Hand h, HandScore hs, ArrayList<eCardNo> wilds) {
		...
		ArrayList<Card> kickers = new ArrayList<Card>();
		if (h.contains(wildRanks) || (h.getCardsInHand().get(eCardNo.FirstCard.getCardNo()).geteRank() == h.getCardsInHand() 
				.get(eCardNo.SecondCard.getCardNo()).geteRank())) {
			isPair = true;
